...
export class ClientDashboardComponent implements OnInit, OnDestroy {
  ...
  filterBookings(): void {
    this.inProgressBookings = this.bookingHistory.filter((booking: Booking) => booking.status === 'Pending' || booking.status === 'Confirmed' || booking.status === 'InProgress');
    this.previousBookings = this.bookingHistory.filter((booking: Booking) => booking.status === 'Completed' || booking.status === 'Cancelled' || booking.status === 'Rejected');
    this.cdr.detectChanges();
    this.loaderService.hide();
  }
